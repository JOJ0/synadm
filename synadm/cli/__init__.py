...
@click.option(
    "--user", "-u", type=str, help="admin user for accessing the Synapse admin API's"
)
@click.option(
    "--token",
    "-t",
...
def config_cmd(helper, user, token, base_url, admin_path, output, timeout):
    ...
    if helper.batch:
        if not all([user, token, base_url, admin_path, output, timeout]):
            click.echo("Missing config options for batch configuration!")
            raise SystemExit(3)
        else:
    ...
    helper.write_config(
        {
            ...
            "base_url": click.prompt(
                ...
                default=base_url
                if base_url
                else helper.config.get("base_url", base_url),
            ),
            "admin_path": click.prompt(
                "Synapse admin API path",
                default=admin_path
                if admin_path
                else helper.config.get("admin_path", admin_path),
            ),
            "format": click.prompt(
            ...
            "timeout": click.prompt(
                ...
                default=timeout if timeout else helper.config.get("timeout", timeout),
            ),
        }
    )
    if not helper.load():
