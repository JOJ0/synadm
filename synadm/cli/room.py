...
def members(helper, room_id):
    ...
    if room_members is None:
        ...
        raise SystemExit(1)
    if helper.output_format == "human":
        click.echo("Total members in room: {}".format(room_members["total"]))
...
@click.pass_obj
@click.pass_context
def delete(ctx, helper, room_id, new_room_user_id, room_name, message, block, no_purge):
    ...
    if sure:
        room_del = helper.api.room_delete(
            room_id, new_room_user_id, room_name, message, block, no_purge
        )
        if room_del is None:
...
def search_room_cmd(ctx, search_term, from_, limit):
    ...
    ctx.invoke(list_room_cmd, from_=from_, limit=limit, name=search_term.lower())
    click.echo("Room search results for '{}':".format(search_term.capitalize()))
    ctx.invoke(list_room_cmd, from_=from_, limit=limit, name=search_term.capitalize())
