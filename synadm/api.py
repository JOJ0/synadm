...
class SynapseAdmin:
    ...
    def user_list(self, _from, _limit, _guests, _deactivated, _name, _user_id):
        ...
        return self.query(
            ...
            params={
                "from": _from,
                "limit": _limit,
                "guests": (str(_guests).lower() if isinstance(_guests, bool) else None),
    ...
    def user_deactivate(self, user_id, gdpr_erase):
        """Delete a given user"""
        return self.query(
            "post", f"v1/deactivate/{user_id}", data={"erase": gdpr_erase}
        )
    ...
    def user_modify(
        self, user_id, password, display_name, threepid, avatar_url, admin, deactivation
    ):
    ...
    def room_delete(
        self, room_id, new_room_user_id, room_name, message, block, no_purge
    ):
    ...
    def room_media_quarantine(self, room_id):
        ...
        return self.query("post", f"v1/room/{room_id}/media/quarantine", data={})

    def user_media_quarantine(self, user_id):
        """Quarantine all local and remote media of a user"""
        return self.query("post", f"v1/user/{user_id}/media/quarantine", data={})

    def media_delete(self, server_name, media_id):
        """Delete a specific (local) media_id"""
        return self.query("delete", f"v1/media/{server_name}/{media_id}/", data={})

    def media_delete_by_date_or_size(
        self, server_name, before_ts, size_gt=None, keep_profiles=None
    ):
        """Delete local media by date and/or size FIXME and/or?"""
        return self.query(
            "post",
            f"v1/media/{server_name}/delete",
            data={},
            params={
    ...
    def media_protect(self, media_id):
        ...
        """
        return self.query("post", f"v1/media/protect/{media_id}/", data={})

    def purge_remote_media(self, user_id, before_ts):
        """Purge old cached remote media"""
        return self.query(
            "post",
            f"v1/purge_media_cache/{before_ts}",
            data={},
